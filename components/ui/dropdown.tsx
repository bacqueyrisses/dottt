  ...
  return (
    <Headless.MenuItems
      ...
      className={clsx(
        ...
        // Base styles
        "isolate w-max rounded-lg p-2",
        // Invisible border that is only visible in `forced-colors` mode for accessibility purposes
        "outline outline-1 outline-transparent focus:outline-none",
        // Handle scrolling when menu won't fit in viewport
        "overflow-y-auto",
        // Popover background
        "backdrop-blur-xl bg-[#242424]",
        // Shadows
...
export function DropdownItem({
  ...
  let classes = clsx(
    className,
    // Base styles
    "group cursor-pointer rounded-md px-3.5 py-2.5 focus:outline-none sm:px-3 sm:py-1.5",
    // Text styles
    "text-left text-base/6  sm:text-sm/6 text-white forced-colors:text-[CanvasText]",
    // Focus
    "data-[focus]:underline data-[focus]:text-white",
    // Disabled state
